...
def plotCoverageToSample(table, cutoff=0):
    ...
    plt.plot(table[:, 0], table[:, 1:7], 'r-', linewidth=2.0)
...
def plotRegionToSample(table, cutoff=0):
    ...
    ax.xaxis.set_ticks_position('bottom')
    ax.yaxis.set_ticks_position('left')
    plt.plot(table[:, 0], table[:, 7:13], 'g-', linewidth=2.0)
...
def plotLengthToSample(table, cutoff=0):
    ...
    ax.xaxis.set_ticks_position('bottom')
    ax.yaxis.set_ticks_position('left')
    plt.plot(table[:, 0], table[:, 13:19], 'b-', linewidth=2.0)
...
my_data = np.genfromtxt('H3K4me3Together300.csv', delimiter=',')
